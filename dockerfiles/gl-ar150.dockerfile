...
FROM plippe/rustc-target:rustup

RUN rustup target add mipsel-unknown-linux-musl

RUN apt-get update && \
    apt-get install -y xz-utils && \
    mkdir -p /opt/openwrt_sdk/ && \
    cd /opt/openwrt_sdk/ && \
    curl -o archive.tar.xz https://downloads.openwrt.org/releases/17.01.4/targets/ar71xx/generic/lede-sdk-17.01.4-ar71xx-generic_gcc-5.4.0_musl-1.1.16.Linux-x86_64.tar.xz && \
    tar -xf archive.tar.xz --strip-components=1 && \
